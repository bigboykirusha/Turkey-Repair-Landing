(() => {
  ...
  window.addEventListener("load", function (e) {
    !(async function () {
      if (document.querySelector(".swiper")) {
        ...
        async function n(e) {
          let t = await fetch(
              "https://jsonplaceholder.typicode.com/todos/" + (29 - e),
              { method: "GET" }
            ),
            s = await t.json();
          i[e] = Object.values(s);
        }
        for (let l = 0; l < 3; l++) n(l);
        let r = document.querySelectorAll(".item__subtitle");
        function a() {
          const t = e.realIndex;
          for (let e = 0; e < s.length; e++)
            (s[e].style.color = "rgba(255,255,255,0.3)"),
              (r[e].textContent = i[t][e]);
  ...
  const re = document.querySelector(".control__video-button"),
    ae = document.querySelector(".control__layout"),
    le = document.querySelector(".control__play");
  le.addEventListener("click", function () {
    ae.classList.remove("video-active"),
      re.classList.remove("video-active"),
      le.pause();
  }),
    re.addEventListener("click", function () {
      ae.classList.add("video-active"),
        re.classList.add("video-active"),
        le.play();
    });
  const oe = document.querySelectorAll(".realize__item");
  let de = document.querySelectorAll(".realize__image"),
    ce = [
      [
        "img/realize1.jpg",
        "img/realize2.jpg",
  ...
  function pe(e) {
    for (let e = 0; e < oe.length; e++) oe[e].style.color = "#FFFFFF";
    oe[e].style.color = "#E3B873";
    for (let t = 0; t < de.length; t++) de[t].setAttribute("src", ce[e][t]);
  }
  for (let e = 0; e < oe.length; e++)
    oe[e].addEventListener("click", () => {
      pe(e);
    });
})();
